...
static std::string ip = "255.255.255.255";
	...
	GGKServerDataGetter getter, GGKServerDataSetter setter)
{
	...
	.gattServiceEnd()
      ...
      .gattServiceBegin("ip", GATT_SERVICE_UUID)
            .gattCharacteristicBegin("addr", GATT_GET_IP_UUID, {"read"})
                  .onReadValue(CHARACTERISTIC_METHOD_CALLBACK_LAMBDA{
                        self.methodReturnValue(pInvocation, ip, true);
                  })
            .gattCharacteristicEnd()
      .gattServiceEnd();
